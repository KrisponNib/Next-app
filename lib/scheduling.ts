...
export function candidateTimes(schedule:LessonSchedule,date:Date){
  ...
  for(let t=mins(w.start);t+schedule.lessonMinutes<=mins(w.end);t+=schedule.slotIntervalMinutes){
    const x=asTime(t);
    if(!bookingConflicts(schedule.bookings,localDateKey(date),x,schedule.lessonMinutes)&&!recurringConflict(schedule.recurringLessons||[],date.getDay(),x,schedule.lessonMinutes))out.push(x)
  }
  return out
}
