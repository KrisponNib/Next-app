...
export function upcomingBookableDays(schedule:LessonSchedule,now=new Date()){const out:{date:string;weekday:number;dayName:string;displayDate:string;times:string[]}[]=[];const parts=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Jerusalem",year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(now);const part=(type:string)=>Number(parts.find(p=>p.type===type)!.value);const start=new Date(part("year"),part("month")-1,part("day"),12);const max=Math.max(7,schedule.advanceDays||14);for(let i=0;i<=max&&out.length<3;i++){const d=new Date(start);d.setDate(start.getDate()+i);if(!schedule.availability.some(a=>a.weekday===d.getDay()&&a.enabled))continue;const times=suggestedTimes(schedule,d,now.getTime());if(times.length)out.push({date:localDateKey(d),weekday:d.getDay(),dayName:HEBREW_DAYS[d.getDay()],displayDate:`${d.getDate()}.${d.getMonth()+1}`,times})}return out}
